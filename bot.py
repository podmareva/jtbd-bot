...
async def start_jtbd(cid, sess, ctx):
    ...
    prompt = (
        "На основе распаковки, позиционирования и всех продуктов составь РОВНО 5 ключевых сегментов целевой аудитории (ЦА), строго по шаблону:\n\n"
        "Каждый сегмент оформляй вот так (смотри пример, соблюдай Markdown-разметку!):\n\n"
        "*Сегмент 1: Любители уюта и красоты*\n"
        "\n"
        "__JTBD:__ Создать уютное и красивое пространство для себя и близких, чтобы чувствовать себя комфортно и радостно.\n"
        "\n"
        "__Потребности:__ Уют, красота, эстетическое удовольствие, создание атмосферы.\n"
        "\n"
        "__Боли:__ Недостаток времени на создание уюта, нехватка идей для декора, стресс от некомфортного интерьера.\n"
        "\n"
        "__Решения:__ Услуги по дизайну интерьера, предоставление дизайн-проектов помещений, консультации по созданию уютного и стильного интерьера.\n"
        "\n"
        "__Темы для контента:__ \"5 способов создать уют в доме\", \"Как выбрать цветовую гамму для интерьера\", \"ТОП-10 украшений для создания уюта\", \"Дизайн интерьера: тренды и идеи\", \"Как сделать атмосферу в спальне\".\n"
        "\n"
        "__Психографика:__ Ценят уют, комфорт, красоту, стремятся создать особую атмосферу в своём доме.\n"
        "\n"
        "__Поведенческая сегментация:__ Активно изучают дизайн интерьера, следят за интерьерными новинками, делятся своими решениями в социальных сетях.\n"
        "\n"
        "__Оффер:__ Консультация и подбор идей для уютного дома от профессионального дизайнера.\n\n"
        "Оформи строго 5 таких сегментов, каждый — отдельным блоком. Не сокращай, не ограничивай себя по объёму, каждый критерий делай отдельной строкой. "
        "Название сегмента — *жирным*, критерии (__JTBD__, __Боли__ и т.д.) — подчёркнутыми, между критериями делай пустую строку. "
        "Используй только текст с Markdown-разметкой. Не используй таблицы и списки. Ответ только на русском языке."
        + style_note +
        "\n\nИсходная информация:\n" + ctx_text
    )
    resp = openai.ChatCompletion.create(
...
async def handle_more_jtbd(update, ctx):
    ...
    prompt = (
        "Добавь ещё 3 неочевидных сегмента целевой аудитории (ЦА), строго по шаблону:\n\n"
        "Каждый сегмент оформляй вот так (Markdown-разметка, пример ниже):\n\n"
        "*Сегмент 6: Ценители уюта и необычных решений*\n"
        "\n"
        "__JTBD:__ Найти идеи для создания стильного и уютного пространства, чтобы проявить индивидуальность.\n"
        "\n"
        "__Потребности:__ Атмосфера, комфорт, оригинальные детали, простота реализации.\n"
        "\n"
        "__Боли:__ Неумение реализовать задумку самостоятельно, страх ошибок в декоре.\n"
        "\n"
        "__Решения:__ Видеоинструкции, подбор материалов, сопровождение на всех этапах.\n"
        "\n"
        "__Темы для контента:__ \"5 нестандартных решений для малогабаритной квартиры\", \"Как объединить разные стили\", \"ТОП-10 уютных аксессуаров\", \"Провальные идеи декора: как не повторить\", \"Где искать вдохновение\".\n"
        "\n"
        "__Психографика:__ Интересы — интерьер, Pinterest, скандинавский стиль; ценности — индивидуальность, комфорт; страхи — показаться банальным.\n"
        "\n"
        "__Поведенческая сегментация:__ Активно ищут необычные идеи, участвуют в челленджах, делятся примерами, смотрят блоги о дизайне.\n"
        "\n"
        "__Оффер:__ Создай уют без ошибок — получи подборку идей и бесплатную консультацию по твоей задумке!\n\n"
        "Оформи строго 3 таких новых неочевидных сегмента, каждый — отдельным блоком. Не сокращай, не ограничивай себя по объёму, каждый критерий делай отдельной строкой. "
        "Название сегмента — *жирным*, критерии (__JTBD__, __Боли__ и т.д.) — подчёркнутыми, между критериями делай пустую строку. "
        "Используй только текст с Markdown-разметкой. Не используй таблицы и списки. Ответ только на русском языке."
        + style_note +
        "\n\nИсходная информация:\n" + ctx_text
