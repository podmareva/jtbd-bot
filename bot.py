...
async def get_feedback(text):
    prompt = (
        f"Вот, что рассказал пользователь:\n\"{text}\"\n\n"
        "Дай обратную связь по-дружески. Если ответ получился спорный, неполный или странный — честно скажи, но мягко.\n"
        "Если хороший — поддержи, но без лести и шаблонов. Ты как ментор или коуч, но тёплый и честный."
    )
    try:
